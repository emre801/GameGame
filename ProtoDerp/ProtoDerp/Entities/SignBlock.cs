...
namespace ProtoDerp
{
    public class SignBlock : EntityBlock
    {
        ...
        public SignBlock(Game g, Arena a, Vector2 pos, int playerNum, String spriteNumber, float rotation, float width, float height)
            : base(g)
        {
        ...
        public bool isInWater()
        {
            ...
            if (waterRec.Contains(playerRec) || waterRec.Intersects(playerRec))
            {
                if ((game.playerOneInput.isAPressed() || keyInput.IsNewKeyPressed(Keys.Enter)) && player.onGround && Math.Abs(player.body.LinearVelocity.X)<0.001f)
                {
                    game.isReadingText = true;
                }
                if(Math.Abs(player.body.LinearVelocity.X)<0.001f)
                    game.isNextToSign = true;
            }
            else
            {
                game.isNextToSign = false;
        ...
        public override void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            if (game.isNextToSign)
            {
                ...
                spriteBatch.Draw(aButt.index, new Rectangle((int)ConvertUnits.ToDisplayUnits(body.Position.X)+10, (int)ConvertUnits.ToDisplayUnits(body.Position.Y)-35,
                    (int)aButt.index.Width, (int)aButt.index.Height), null, drawColor, body.Rotation, origin, SpriteEffects.None, 0f);
